...
class Hand {
    ...
    Hand([string]$RawHand) {
        ...
        if ($this.HashCard.Keys -contains "J") {
            switch (($this.HashCard.GetEnumerator() | Where-Object {$_.Key -match "J"}).Value) {
                5 { 
                ...
                3 { 
                    if ($this.HashCard.Keys.Count -eq 2) {
                        $this.Combination = [Combination]::FiveOfAKind
                    }
                    else {
                ...
                2 { 
                    ...
                    elseif ($this.HashCard.Keys.Count -eq 3) {
                        $this.Combination = [Combination]::FourOfAKind
                    }
                    else  {
                ...
                1 { 
                    ...
                    elseif (($this.HashCard.Keys.Count -eq 3) -and (($this.HashCard.GetEnumerator() | Where-Object {$_.key -notmatch "J"}).Value -contains 3)) {
                        $this.Combination = [Combination]::FourOfAKind
                    }
                    elseif (($this.HashCard.Keys.Count -eq 3)) {
                        $this.Combination = [Combination]::FullHouse
                    }
                    elseif (($this.HashCard.Keys.Count -eq 4)) {
                        $this.Combination = [Combination]::Brelan
                    }
                    else  {
